...
class Door(SingleArmEnv):
    ...
    def __init__(
        ...
        generalization=False,  # Flag for generalization experiments
    ):
    ...
    def reward(self, action=None):
        ...
        if self._check_success():
            reward = 300.0 if self.use_latch else 1.0
        ...
        # else, we consider only the case if we're using shaped rewards
            ...
            if dist < 0.1:
                ...
                if self.use_latch:
                    ...
                    reward += np.clip(0.25 * handle_qpos / (0.5 * np.pi), 0.0, 0.25)

                # Add reward for rotating the door
                hinge_qpos = self.sim.data.qpos[self.hinge_qpos_addr]
                reward += np.clip(0.5 * hinge_qpos / 0.3, 0.0, 0.5)
    ...
    def _post_action(self, action):
        ...
        if self.use_latch:
            # early termination upon success
            success = self._check_success()
            self.done = self.done or success
    ...
    def _load_model(self):
        ...
        if self.placement_initializer is not None:
            ...
            self.placement_initializer = UniformRandomSampler(
                ...
                x_range=[0.06, 0.10] if self.generalization else [0.07, 0.07],
                y_range=[-0.01, 0.01] if self.generalization else [0.00, 0.00],
    ...
    def _check_success(self):
        ...
        dist = np.linalg.norm(self._gripper_to_handle)
        hinge_qpos = self.sim.data.qpos[self.hinge_qpos_addr]
        return dist < 0.1 and hinge_qpos > 0.3
