...
class SingleArmTactile(Manipulator):
    """
    ...
    def setup_observables(self):
        ...
        if self.has_gripper:
            ...
            modality = f"{pf}tactile_right"
            @sensor(modality=modality)
            def gripper_tactile_right(obs_cache):
                tactile_right = self.sim.data.sensor(self.gripper.important_sensors["touch_right"]).data.reshape((3, 32, 32))
                tactile_right = tactile_right[[1, 2, 0]]
                tactile_right = np.sign(tactile_right) * np.log(1 + np.abs(tactile_right))
                return tactile_right

            modality = f"{pf}tactile_left"
            @sensor(modality=modality)
            def gripper_tactile_left(obs_cache):
                tactile_left = self.sim.data.sensor(self.gripper.important_sensors["touch_left"]).data.reshape((3, 32, 32))
                tactile_left = tactile_left[[1, 2, 0]]
                tactile_left = np.sign(tactile_left) * np.log(1 + np.abs(tactile_left))
                return tactile_left

            sensors += [gripper_qpos, gripper_qvel, gripper_tactile_right, gripper_tactile_left]
            names += [f"{pf}gripper_qpos", f"{pf}gripper_qvel", f"{pf}tactile_right", f"{pf}tactile_left"]
